...
RouteMod::RouteMod(uint8_t mod, uint64_t id, std::vector<Match*> matches, std::vector<Action*> actions) {
    set_mod(mod);
    set_id(id);
    set_matches(matches);
    set_actions(actions);
...
void RouteMod::add_match(Match* match) {
...
void RouteMod::add_action(Action* action) {
...
void RouteMod::from_BSON(const char* data) {
    ...
    set_matches(MatchList::to_vector(obj["matches"]));
    set_actions(ActionList::to_vector(obj["actions"]));
}
...
const char* RouteMod::to_BSON() {
    ...
    _b.append("matches", MatchList::to_BSON(get_matches()));
    _b.append("actions", ActionList::to_BSON(get_actions()));
    mongo::BSONObj o = _b.obj();
    char* data = new char[o.objsize()];
    memcpy(data, o.objdata(), o.objsize());
    return data;
