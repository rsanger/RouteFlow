...
"string": "{0}.String()",
"match[]": "MatchList::to_vector({0})",
"action[]": "ActionList::to_vector({0})",
}
...
def genH(messages, fname):
    ...
    for (name, msg) in messages:
        ...
        for t, f in msg:
            ...
            if t[-2:] == "[]":
                t2 = t[0:-2]
                g.addLine("void add_{0}({1} {0});".format(t2, typesMap[t2]))
            g.blankLine()
...
def genCPP(messages, fname):
    ...
    for name, msg in messages:
        ...
        for t, f in msg:
            ...
            if t[-2:] == "[]":
                t2 = t[0:-2]
                g.addLine("void {0}::add_{1}({2} {1}) {{".format(name, t2, typesMap[t2]))
                g.increaseIndent()
